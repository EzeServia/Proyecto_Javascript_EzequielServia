//funcion principal del sistema de cobro
...
//En ésta funcion divido el costo del producto para llevarlo al valor en dólares
...
function cobrador() {
  ...
  while (forma_pago != "salir") {
    /*Este código se repite indefinidamente hasta que el usuario
      ingrese "Salir" en el siguiente prompt*/
    alert(
      "Bienvenido al sistema de cobros... COBRO BIEN, por favor, indique de que forma quiere abonar...  "
    );
    forma_pago = prompt("Ingrese su forma de pago.\n-pesos\n-dolares\n-salir");
    // Convierte el prompt del usuario en minúscula para aseguranos que siempre llegue asi.
    ...
    switch (forma_pago) {
      ...
      case "dolares":
        alert(
          "El Cambio de divisas se realiza con un valor DOLAR BLUE de $730 "
        );
        costo_producto = parseInt(
          prompt("Ingresá el costo del producto en pesos:")
        );
        pago_dolar = parseInt(prompt("¿Con cuantos dolares abona ?"));
        forma_pago_dolar = prompt(
          "Queres recibir tu vuelto en dolares ?\n- SI\n- NO"
        );
        if (forma_pago_dolar == "SI" || forma_pago_dolar == "si") {
          alert(
            "Su vuelto en DOLARES es...USD" +
              cobrar_d(costo_producto, pago_dolar) +
              "\nMuchas gracias por elegirnos."
          );
        } else {
          alert(
            "Su vuelto en PESOS es... $" +
              cobrar_dolar(costo_producto, pago_dolar) +
              "\nMuchas gracias por elegirnos."
          );
        }
        break;
